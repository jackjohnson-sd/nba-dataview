    ...
    for r in rects:
        ...
        for n, code in enumerate(codes):
            ...
            if n == 0 and r["side"] == "o":
                ...
                # centred over the event list: the list's own midpoint,
                # which is half its length out from the centre line
                _mid = (CENTRE - len(codes) * SEG_W / 2 if r["dir"] < 0
                        else CENTRE + len(codes) * SEG_W / 2)
                _pos = f'left:{_mid:.2f}%;'
                parts.append(
...
/* the game time, centred over its own event list and lifted above it */
